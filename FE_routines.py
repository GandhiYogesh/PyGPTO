...
def FE_assemble_BC(FE, OPT, GEOM):
    ...
    for idisp in range(0, FE['BC']['n_pre_disp_dofs']):
        idx = FE['dim'] * FE['BC']['disp_node'][idisp] + \
            FE['BC']['disp_dof'][idisp]
...
def FE_assemble_stiffness_matrix(FE, OPT, GEOM):
    ...
    simp_rho_e = np.tile(OPT['simp']['elem_dens'][:, None, None],
                         (1, FE['n_edof'], FE['n_edof'])).transpose((1, 2, 0))

    # Ersatz material: (Eq. (7))
    penalized_Ke = penalized_rho_e * \
        FE['Ke_proj'] + simp_rho_e * FE['simp']['Ke']
    # penalized_Ke = FE['Ke_proj'] + simp_rho_e * FE['simp']['Ke']
...
def FE_compute_constitutive_matrices(FE, OPT, GEOM):
    ...
    # compute the elastic matrix for the bar-material
    Cbar = FE_compute_constitutive_matrix(
        FE, OPT, GEOM, FE['material']['E'], FE['material']['nu'])

    FE['material']['C'] = Cbar

    # compute the elastic matrix for the matrix-material
    Cmat = FE_compute_constitutive_matrix(
        FE, OPT, GEOM, FE['simp']['matrix']['E'], FE['simp']['matrix']['nu'])

    FE['simp']['C'] = Cmat

    Cvoid = FE_compute_constitutive_matrix(
        FE, OPT, GEOM, FE['void']['Emin'], FE['void']['nu'])
...
def B(GN):
    if 2 == GN.shape[0]:
        ...
        B_ = np.array(((GN[0, 0], 0, 0, GN[0, 1], 0, 0, GN[0, 2], 0, 0, GN[0, 3], 0, 0, GN[0, 4], 0, 0, GN[0, 5], 0, 0, GN[0, 6], 0, 0, GN[0, 7], 0, 0),
                       (0, GN[1, 0], 0, 0, GN[1, 1], 0, 0, GN[1, 2], 0, 0, GN[1, 3], 0,
                        0, GN[1, 4], 0, 0, GN[1, 5], 0, 0, GN[1, 6], 0, 0, GN[1, 7], 0),
                       (0, 0, GN[2, 0], 0, 0, GN[2, 1], 0, 0, GN[2, 2], 0, 0, GN[2, 3],
                        0, 0, GN[2, 4], 0, 0, GN[2, 5], 0, 0, GN[2, 6], 0, 0, GN[2, 7]),
                       (GN[1, 0], GN[0, 0], 0, GN[1, 1], GN[0, 1], 0, GN[1, 2], GN[0, 2], 0, GN[1, 3], GN[0, 3], 0,
                        GN[1, 4], GN[0, 4], 0, GN[1, 5], GN[0, 5], 0, GN[1, 6], GN[0, 6], 0, GN[1, 7], GN[0, 7], 0),
                       (0, GN[2, 0], GN[1, 0], 0, GN[2, 1], GN[1, 1], 0, GN[2, 2], GN[1, 2], 0, GN[2, 3], GN[1, 3],
                        0, GN[2, 4], GN[1, 4], 0, GN[2, 5], GN[1, 5], 0, GN[2, 6], GN[1, 6], 0, GN[2, 7], GN[1, 7]),
...
def FE_compute_element_stiffness(FE, OPT, GEOM, C):
    ...
    Ke = np.zeros((FE['dim']*2**FE['dim'], FE['dim']
                  * 2**FE['dim'], FE['n_elem']))
...
def FE_init_element_stiffness(FE, OPT, GEOM):
    ...
    FE['simp']['Ke'] = FE_compute_element_stiffness(FE, OPT, GEOM, FE['simp']['C']).reshape(
        (n_edof, n_edof, FE['n_elem']))
...
def FE_init_partitioning(FE, OPT, GEOM):
    ...
    if 2 == FE['dim']:
        FE['fixeddofs'][2*FE['BC']['disp_node']
                        [FE['BC']['disp_dof'] == 0]] = True  # set prescribed x1 DOFs
        FE['fixeddofs'][2*FE['BC']['disp_node']
                        [FE['BC']['disp_dof'] == 1] + 1] = True   # set prescribed x2 DOFs
    elif 3 == FE['dim']:
        FE['fixeddofs'][3*FE['BC']['disp_node']
                        [FE['BC']['disp_dof'] == 0]] = True  # set prescribed x1 DOFs
        FE['fixeddofs'][3*FE['BC']['disp_node']
                        [FE['BC']['disp_dof'] == 1] + 1] = True  # set prescribed x2 DOFs
        FE['fixeddofs'][3*FE['BC']['disp_node']
                        [FE['BC']['disp_dof'] == 2] + 2] = True   # set prescribed x3 DOFs
...
def FE_solve(FE, OPT, GEOM):
    ...
    if 'direct' == FE['analysis']['solver']['type']:
        if FE['analysis']['solver']['use_gpu'] == True:
            print(
                'GPU solver selected, but only available for iterative solver, solving on CPU.')
