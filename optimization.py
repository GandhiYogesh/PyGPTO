...
from simp_dens import run_simp
...
def init_optimization(FE, OPT, GEOM):
    ...
    if not ('elem_r' in OPT['parameters']):
        ...
        OPT['parameters']['elem_r'] = np.sqrt(
            FE['dim'])/2 * FE['elem_vol']**(1/FE['dim'])
    ...
    # such that dv(point_dv) = point
    OPT['point_dv'] = np.arange(0, FE['dim']*GEOM['n_point'])
    ...
    pt_dv = OPT['point_dv'].reshape(
        (FE['dim'], GEOM['n_point']), order='F').copy()
...
def runopt(FE, OPT, GEOM, x0, obj, nonlcon):
    ...
    lb_bar = np.vstack(
        (lb_point, lb_point, np.array((lb_size, lb_radius))[:, None]))
    ub_bar = np.vstack(
        (ub_point, ub_point, np.array((ub_size, ub_radius))[:, None]))
    ...
    if 'default' == OPT['options']['optimizer']:
        def output(x, state):
            ...
            if state.nit == 1:
                ...
                history['fval'] = np.concatenate(
                    (history['fval'], state['fun'][:, None]), axis=1)
                history['fconsval'] = np.concatenate(
                    (history['fconsval'], state['constr'][0][:, None]), axis=1)
                # here we make x into a column vector
                history['x'] = np.concatenate(
                    (history['x'], x[:, None]), axis=1)

            folder, baseFileName = os.path.split(
        ...
        ml_step = OPT['options']['move_limit'] * \
            abs(ub - lb)  # Compute move limits once
        ...
        while kktnorm > OPT['options']['kkt_tol'] and \
            ...
            print('It. ' + str(iter) + ', Obj= ' + str(f0val) +
                  ', ConsViol = ' + str(max(max(fval, np.zeros((ncons, 1))))))
            print('\tKKT-norm = ' + str(kktnorm) +
                  'DV norm change ' + str(dv_step_change))

            # Concatenate current point and obj value with history
            history['fval'] = np.concatenate(
                (history['fval'], f0val[:, None]), axis=1)
            history['fconsval'] = np.concatenate(
                (history['fconsval'], fval[:, None]), axis=1)
            history['x'] = np.concatenate(
                (history['x'], x[:, None]), axis=1)  # here we make x into a
